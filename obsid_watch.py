...
def find_changed_files(root_folder, days):
    ...
    for root, dirs, files in os.walk(root_folder):
        ...
        for f in files:
            ...
            if os.path.getmtime(file_path) > time.time() - 86400 * days:  # файлы измененные за последний день
...
def main(action, root_folder, output_zip_path, days):
    output_folder = root_folder
    ensure_directory_exists(output_zip_path)

    if action == 'p':
        changed_files = find_changed_files(root_folder, days)
        if changed_files:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--days', type=int, help='Количество дней для отслеживания изменений', default=1)
    ...
    main(args.action, root_folder, output_zip_path, days)
